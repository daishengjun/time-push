...
def get_weather(province, city):
    ...
    headers = {
        "Referer": "http://www.weather.com.cn/weather1d/{}.shtml".format(101280101),
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) '
                      'AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36'
    }
    url = "http://d1.weather.com.cn/dingzhi/{}.html?_={}".format(101280101, t)
...
def caihongpi():
    if (Whether_caihongpi!=False):
        try:
            ...
            params = urllib.parse.urlencode({'key':'6a7edca7369f3983b9ea32b9f79302c3'})
...
def lucky():
    if ( Whether_lucky!=False):
        try:
            conn = http.client.HTTPSConnection('api.tianapi.com')  #接口域名
            params = urllib.parse.urlencode({'key':'6a7edca7369f3983b9ea32b9f79302c3','astro':'aquarius'})
            headers = {'Content-type':'application/x-www-form-urlencoded'}
...
def tip():
    if (Whether_tip!=False):
        try:
            conn = http.client.HTTPSConnection('api.tianapi.com')  #接口域名
            params = urllib.parse.urlencode({'key':'6a7edca7369f3983b9ea32b9f79302c3','city':'广州市'})
            headers = {'Content-type':'application/x-www-form-urlencoded'}
...
if __name__ == "__main__":
    try:
        with open("tuisong\config.json", encoding="utf-8") as f:
            config = eval(f.read())
    ...
    # 公众号推送消息
    for user in users:
        send_message(user, accessToken, city, weather, max_temperature, min_temperature, pipi, lizhi, pop, tips, health_tip = health(), lucky_ = lucky())
    import time
    time_duration = 3.5
